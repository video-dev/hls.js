...
declare function eventCallback(event: string, data: HlsData): void;
...
interface Hls {
   ...
   */
  currentLevel(level: number): void;
  /**
   ...
   */
  autoLevelCapping(level: number): void;
  /**
  ...
  startLevel(): number;
  /**
   * set start level index (level of first fragment that will be played back)
   *      if -1: automatic start level selection, playback will start from level matching download bandwidth (determined from download of first segment)
   */
   ...
   * if set to false, an explicit API call (hls.startLoad(startPosition=-1)) will be needed to start quality level/fragment loading.
   */
  autoStartLoad(value: boolean): void;
  /**
   *  hls.js config
   */
  config(): HlsConfig;
   ...
   * set to -1 for automatic level selection
   */
  nextLevel(qualityLevel: number): Level;
  /**
   ...
   * set to -1 for automatic level selection
   */
  loadLevel(qualityLevel: number): Level;
  /**
   ...
   */
  nextLoadLevel(qualityLevel: number): Level;
  /**
...
interface HlsConfig {
  ...
  */
   ...
   */
  fetchSetup?: (context, initParams) => Request;
  /**
...
interface AbrController {
  /**
   * gets capping/max level value that could be used by ABR Controller
   */
  autoLevelCapping(): number;
  /**
   * set capping/max level value that could be used by ABR Controller
   */
  autoLevelCapping(value: number);
  /**
   ...
   *  - currently used for emergency switch down
   */
  nextAutoLevel(value: number): void;
...
interface LoaderCallbacks {
  onSuccessCallback: (response: LoaderResponse, stats: LoaderStats, context: LoaderContext) => void;
  onProgressCallback: (stats: LoaderStats, context: LoaderContext, data: string | ArrayBuffer) => void;
  onErrorCallback: (error: LoaderError, context: LoaderContext) => void;
  onTimeoutCallback: (stats: LoaderStats, context: LoaderContext) => void;
}
