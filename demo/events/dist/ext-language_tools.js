  ...
  function (require, exports, module) {
    ...
    (function () {
      ...
      this.tmStrFormat = function (str, ch, editor) {
        ...
        var fmtTokens =
          typeof ch.fmt == 'string'
            ? this.tokenizeTmSnippet(ch.fmt, 'formatString')
            : ch.fmt;
        var _self = this;
      ...
      this.resolveVariables = function (snippet, editor) {
        ...
        for (var i = 0; i < snippet.length; i++) {
          ...
          if (ch.text) {
            var value =
              this.getVariableValue(editor, ch.text, indentation) + '';
      ...
      this.insertSnippetForSelection = function (editor, snippetText) {
        ...
        for (var i = 0; i < tokens.length; i++) {
          ...
          tokens.splice.apply(
            tokens,
            [i + 1, Math.max(0, i1 - i)].concat(value, p)
          );
    ...
    (function () {
      ...
      this.updateLinkedFields = function () {
        ...
        for (var i = 0; i < ts.length; i++) {
          ...
          var fmt = exports.snippetManager.tmStrFormat(
            text,
            original,
            this.editor
          );
      ...
      this.keyboardHandler.bindKeys({
        Tab: function (editor) {
          if (
            ...
            exports.snippetManager.expandWithTab(editor)
          )
            return;
    ...
    var AcePopup = function (parentNode) {
      ...
      bgTokenizer.$tokenizeRow = function (row) {
        ...
        for (var i = 0; i <= filterText.length; i++) {
          if (
            i != lastI &&
            (data.matchMask & (1 << i) || i == filterText.length)
          ) {
      ...
      popup.setRow = function (line) {
        line = Math.max(
          this.autoSelect ? 0 : -1,
          Math.min(this.data.length, line)
        );
          ...
          if (rect.left < 320) {
            if (popup.isTopdown) {
              ...
              tooltipNode.style.top =
                popup.container.offsetTop - tooltipNode.offsetHeight + 'px';
              tooltipNode.style.left = rect.left + 'px';
              tooltipNode.style.right = '';
              tooltipNode.style.bottom = '';
            }
...
ace.define(
  ...
  function (require, exports, module) {
    ...
    var transformSnippetTooltip = function (str) {
      var record = {};
      return str
        .replace(/\${(\d+)(:(.*?))?}/g, function (_, p1, p2, p3) {
          return (record[p1] = p3 || '');
        })
