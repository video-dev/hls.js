...
describe('parseMediaFragment', function () {
  describe('URLs without fragments', function () {
    it('should return URL as-is when no fragment present', function () {
      ...
      expect(result.url).to.equal('http://example.com/video.m3u8');
      expect(result.temporal).to.be.undefined;
    });
    ...
    it('should handle URLs with query parameters but no fragment', function () {
      ...
      expect(result.url).to.equal('http://example.com/video.m3u8?token=abc123');
      expect(result.temporal).to.be.undefined;
    });
  });
  ...
  describe('Simple temporal fragments', function () {
    ...
    it('should parse fractional seconds', function () {
      ...
      expect(result.temporal).to.deep.equal({ start: 10.5, end: 20.75 });
    });
  });
  ...
  describe('HH:MM:SS format', function () {
    it('should parse hhmmss format', function () {
      ...
      expect(result.temporal).to.deep.equal({ start: 120, end: 210 });
    });
    ...
    it('should parse hhmmss with fractional seconds', function () {
      ...
      expect(result.temporal).to.deep.equal({ start: 5445.5, end: 5700.25 });
    });
    ...
    it('should parse mmss with fractional seconds', function () {
      ...
      expect(result.temporal).to.deep.equal({ start: 1845.5, end: 2100.25 });
    });
    ...
    it('should handle hours > 9', function () {
      ...
      expect(result.temporal).to.deep.equal({ start: 36000, end: 45000 });
    });
  });
  ...
  describe('Edge cases', function () {
    it('should handle zero start time', function () {
      ...
      expect(result.temporal).to.deep.equal({ start: 0, end: 10 });
    });
    ...
    it('should preserve query parameters', function () {
      ...
      expect(result.url).to.equal('http://example.com/video.m3u8?token=abc');
      expect(result.temporal).to.deep.equal({ start: 10, end: 20 });
    });
