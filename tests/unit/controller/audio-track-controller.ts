...
describe('AudioTrackController', function () {
  ...
  describe('clearing expired inactive playlist details', function () {
    ...
    it('deletes expired tracks outside the active group', function () {
      tracks[1].details = liveDetails(120); // inactive expired -> deleted
      tracks[2].details = liveDetails(0); // inactive not expired -> retained

      loadActiveTrack();

      expect(tracks[1].details, 'expired details in inactive group').to.not
        .exist;
      expect(tracks[2].details, 'non-expired details in inactive group').to
        .exist;
    });

    it('deletes expired tracks outside the active group (same id as active track)', function () {
      expect(tracks[0].id, 'colliding id').to.equal(tracks[3].id);
      tracks[0].details = liveDetails(120);

      loadActiveTrack();

      expect(tracks[3].details, 'active track details').to.exist;
      expect(tracks[0].details, 'expired details sharing active id').to.not
        .exist;
    });
  });
});
