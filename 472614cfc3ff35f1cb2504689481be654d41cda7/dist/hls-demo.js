...
Object.defineProperty(exports, "__esModule", { value: true });
var demo_utils_1 = __webpack_require__(/*! ./demo-utils */ "./demo/demo-utils.js");
...
function loadSelectedStream() {
    ...
    hls.on(Hls.Events.FRAG_BUFFERED, function (event, data) {
        ...
        refreshCanvas();
        updateLevelInfo();
        var latency = data.stats.tfirst - data.stats.trequest, parsing = data.stats.tparsed - data.stats.tload, process = data.stats.tbuffered - data.stats.trequest, bitrate = Math.round(8 * data.stats.length / (data.stats.tbuffered - data.stats.tfirst));
    ...
    hls.on(Hls.Events.ERROR, function (event, data) {
        ...
        $('#statisticsOut').text(JSON.stringify(demo_utils_1.sortObject(stats), null, '\t'));
    });
...
function handleVideoEvent(evt) {
    ...
    switch (evt.type) {
        ...
        case 'error':
            ...
            if (evt.type === 'error') {
                var errorTxt = void 0, mediaError = evt.currentTarget.error;
...
function checkBuffer() {
    ...
    if (r) {
        ...
        if (bufEventLen > 1) {
            ...
            var slopeBuf1 = (event1.buffer - event_1.buffer) / (event1.time - event_1.time);
            var slopePos0 = (event0.pos - event1.pos) / (event0.time - event1.time);
            var slopePos1 = (event1.pos - event_1.pos) / (event1.time - event_1.time);
...
function copyMetricsToClipBoard() {
    demo_utils_1.copyTextToClipboard(getMetrics());
}
...
function createTestStream(url, description, live, abr, blacklist_ua) {
    ...
    if (abr === void 0) { abr = true; }
    if (blacklist_ua === void 0) { blacklist_ua = []; }
    return {
        url: url,
