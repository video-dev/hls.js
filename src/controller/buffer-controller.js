...
class BufferController extends EventHandler {
    ...
    onSBUpdateEnd() {
        ...
        let pending = this.segments.reduce(
            (counter, segment) =>
                segment.parent === parent ? counter + 1 : counter,
            0
        );
    ...
    updateMediaElementDuration() {
        ...
        if (this._live === true && config.liveDurationInfinity === true) {
            ...
            logger.log(
                `Updating Media Source duration to ${this._levelDuration.toFixed(
                    3
                )}`
            );
  ...
  */
    flushBuffer(startOffset, endOffset, typeIn) {
        ...
        if (Object.keys(sourceBuffer).length) {
            logger.log(
                `flushBuffer,pos/start/end: ${this.media.currentTime.toFixed(
                    3
            ...
            if (this.flushBufferCounter < this.appended) {
                ...
                */
                                if (
                                    Math.min(flushEnd, bufEnd) - flushStart >
                                    0.5
                                ) {
