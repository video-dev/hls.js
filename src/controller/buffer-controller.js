  ...
  flushLiveBackBuffer () {
    ...
    for (let index = bufferTypes.length - 1; index >= 0; index--) {
      ...
      if (buffered.length > 0 && targetBackBufferPosition > buffered.start(0)) {
        ...
        if (this.removeBufferRange(bufferType, sourceBuffer[bufferType], 0, targetBackBufferPosition)) {
          this.hls.trigger(Event.BUFFER_RANGE_FLUSHED, { startOffset: 0, endOffset: targetBackBufferPosition });
        }
      }
    }
  }
