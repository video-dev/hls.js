...
class StreamController extends TaskLoop {
    ...
    _doTickIdle() {
        ...
        if (
            ...
            (levelDetails.live === true && this.levelLastLoaded !== level)
        ) {
  ...
  */
    ...
    _checkAppendedParsed() {
        ...
        if (
            ...
            if (frag) {
                ...
                let bufferedFrags = this._bufferedFrags.filter(frag => {
                    return BufferHelper.isBuffered(
                        media,
                        (frag.startPTS + frag.endPTS) / 2
                    );
                });
                // push new range
    ...
    _checkBuffer() {
        ...
        if (media && media.readyState) {
            ...
            if (!this.loadedmetadata && buffered.length) {
                ...
                let startPosition = media.seeking
                        ? currentTime
                        : this.startPosition,
    ...
    */
        const media = this.mediaBuffer ? this.mediaBuffer : this.media;
        this._bufferedFrags = this._bufferedFrags.filter(frag => {
            return BufferHelper.isBuffered(
                media,
                (frag.startPTS + frag.endPTS) / 2
            );
