...
class StreamController extends EventHandler {
    ...
    _doTickIdle() {
        ...
        logger.trace(
            `buffer length of ${bufferLen.toFixed(
                3
            )} is below max of ${maxBufLen.toFixed(
                3
    ...
    _ensureFragmentAtLivePoint({
        ...
        */
                frag =
                    fragments[Math.min(fragLen - 1, Math.round(fragLen / 2))];
                logger.log(
    ...
    _findFragment({
        ...
        let fragNext = fragPrevious
            ? fragments.find(fragment => fragment.sn === fragPrevious.sn + 1)
            : undefined;
        ...
        if (frag.decryptdata.uri != null && frag.decryptdata.key == null) {
            ...
            if (frag.loadCounter) {
                ...
                if (
                    ...
                    Math.abs(this.fragLoadIdx - frag.loadIdx) < maxThreshold
                ) {
    ...
    get nextBufferRange() {
        ...
        if (media) {
            ...
            return this.followingBufferRange(
                this.getBufferRange(media.currentTime)
            );
        } else {
            return null;
        }
  ...
  */
    ...
    _checkAppendedParsed() {
        ...
        if (
            ...
            if (frag) {
                ...
                let bufferRange = this.bufferRange.filter(range => {
                    return BufferHelper.isBuffered(
                        media,
            ...
            case ErrorDetails.LEVEL_LOAD_TIMEOUT:
                if (this.state !== State.ERROR) {
                    if (data.fatal) {
                        ...
                        this.state = State.ERROR;
                        logger.warn(
                            `streamController: ${data.details},switch to ${
                                this.state
                            } state ...`
                        );
                    } else {
                    ...
                    if (expectedPlaying) {
                        ...
                        if (!this.stalled) {
                            ...
                            if (
                                ...
                                stalledDuration >
                                    config.lowBufferWatchdogPeriod * 1000
                            ) {
                                // report stalled error once
                                if (!this.stallReported) {
                                    this.stallReported = true;
                                    logger.warn(
                                        `playback stalling in low buffer @${currentTime}`
                                    );
                                    hls.trigger(Event.ERROR, {
                                        type: ErrorTypes.MEDIA_ERROR,
                                        details:
    ...
    */
        const media = this.mediaBuffer ? this.mediaBuffer : this.media;
        this.bufferRange = this.bufferRange.filter(range => {
            return BufferHelper.isBuffered(
                media,
                (range.start + range.end) / 2
            );
