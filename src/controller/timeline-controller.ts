...
class TimelineController extends EventHandler {
  ...
  private config: HlsConfig | null = null;
  private enabled: boolean = true;
  private cues: any = null;
  ...
  onInitPtsFound (data: { id: string, frag: Fragment, initPTS: number}): void {
    ...
    if (unparsedVttFrags.length) {
      ...
      unparsedVttFrags.forEach((frag: any) => {
  ...
  getExistingTrack (trackName: string): TextTrack | null {
    ...
    if (media) {
      for (let i = 0, len = media.textTracks.length; i < len; i++) {
  ...
  createCaptionsTrack (trackName: string): void {
    ...
    if (!media) {
      console.warn('createCaptionsTrack called without attached media');
      return;
    }
  ...
  onMediaAttaching (data: { media: HTMLMediaElement }): void {
  ...
  onMediaDetaching (): void {
    ...
    Object.keys(captionsTracks).forEach((trackName: string) => {
      clearCurrentCues(captionsTracks[trackName]);
  ...
  onManifestLoaded (data: { subtitles: Array<any> }): void {
    this.textTracks = [];
    this.unparsedVttFrags = this.unparsedVttFrags || [];
    this.initPTS = [];
    this.cueRanges = [];

    if (this.config && this.config.enableWebVTT) {
      ...
      this.tracks.forEach((track: any, index: number) => {
        ...
        if (index < inUseTracks.length) {
          ...
          for (let i = 0, len = inUseTracks.length; i < len; i++) {
  ...
  onLevelSwitching (): void {
    this.enabled = this.hls.currentLevel !== -1;
  }

  onFragLoaded (data: { frag: Fragment, payload: any }): void {
  ...
  private _parseVTTs (frag: Fragment, payload: any): void {
    ...
    WebVTTParser.parse(payload, this.initPTS[frag.cc], vttCCs, frag.cc, (cues: any) => {
      ...
      if (currentTrack.mode === 'disabled') {
        hls.trigger(Event.SUBTITLE_FRAG_PROCESSED, { success: false, frag });
        return;
      }
      ...
      hls.trigger(Event.SUBTITLE_FRAG_PROCESSED, { success: true, frag });
    },
    (e: Error) => {
  ...
  onFragDecrypted (data: { frag: Fragment, payload: any}): void {
    const { frag, payload } = data;
  ...
  onFragParsingUserdata (data: { samples: Array<any> }): void {
    if (!this.enabled || (!this.config || !this.config.enableCEA708Captions)) {
      return;
    }
  ...
  extractCea608Data (byteArray: Uint8Array): Array<number> {
    ...
    let ccValid: boolean;
    let ccType: number;
    const actualCCBytes: Array<number> = [];
