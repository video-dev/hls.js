    ...
    set level(newLevel) {
        ...
        if (levels) {
            ...
            if (
                ...
                levels[newLevel].details === undefined
            ) {
    ...
    recoverLevel(errorEvent, levelIndex, levelError, fragmentError) {
        ...
        if (levelError === true) {
            if (this.levelRetryCount + 1 <= config.levelLoadingMaxRetry) {
                ...
                logger.warn(
                    `level controller, ${errorDetails}, retry in ${delay} ms, current retry count is ${
                        this.levelRetryCount
                    }`
                );
            } else {
                logger.error(
                    `level controller, cannot recover from ${errorDetails} error`
                );
                this.currentLevelIndex = null;
                // stopping live reloading timer if any
                this.cleanTimer();
        ...
        if (levelError === true || fragmentError === true) {
            ...
            if (redundantLevels > 1 && level.loadError < redundantLevels) {
                logger.warn(
                    `level controller, ${errorDetails} for level ${levelIndex}: switching to redundant stream id ${
                        level.urlId
                    }`
                );
                ...
                if (this.manualLevelIndex === -1) {
                    ...
                    nextLevel =
                        levelIndex === 0
                            ...
                            : levelIndex - 1;
                    logger.warn(
                        `level controller, ${errorDetails}: switch to ${nextLevel}`
                    );
                    ...
                    logger.warn(
                        `level controller, ${errorDetails}: reload a fragment`
                    );
                    this.currentLevelIndex = null;
                }
            }
