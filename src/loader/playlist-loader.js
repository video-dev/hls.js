...
class PlaylistLoader extends EventHandler {
    ...
    load(url, context) {
        ...
        let maxRetry, timeout, retryDelay, maxRetryDelay;
        ...
        switch (context.type) {
            ...
            default:
                ...
                logger.log(
                    `Playlist loader for ${context.type} ${context.level ||
                        context.id}`
                );
                break;
        }
    ...
    _handleMasterPlaylist(response, stats, context, networkDetails) {
        ...
        let audioTracks = M3U8Parser.parseMasterPlaylistMedia(
            string,
            url,
            'AUDIO',
            audioGroups
        );
        let subtitles = M3U8Parser.parseMasterPlaylistMedia(
            string,
            url,
            'SUBTITLES'
        );
    ...
    _handleSidxRequest(response, context) {
        const sidxInfo = MP4Demuxer.parseSegmentIndex(
        ...
        context.levelDetails.initSegment.rawByteRange =
