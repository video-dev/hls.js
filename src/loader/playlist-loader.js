...
const TARGETDURATION  = '#EXT-X-TARGETDURATION:';
 ...
 class PlaylistLoader {
  ...
  parseLevelPlaylist(string, baseurl) {
    ...
    var currentSN, duration, extinfFound = false;
    var lines = string.split(/\r?\n/);
    var fragments = [];
    lines.forEach(line => {
       if(line.indexOf(SEQNUM) === 0) {
        ...
        duration  = (commaPosition === -1) ? parseFloat(line.substr(FRAGMENT.length)) : parseFloat(line.substr(FRAGMENT.length, commaPosition - FRAGMENT.length));
