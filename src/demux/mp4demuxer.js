...
class MP4Demuxer {
    ...
    static probe(data) {
        ...
        return (
            MP4Demuxer.findBox(
                { data: data, start: 0, end: Math.min(data.length, 16384) },
                ['moof']
            ).length > 0
        );
    }
    ...
    static parseSegmentIndex(initSegment) {
        ...
        for (let i = 0; i < referencesCount; i++) {
            ...
            const subsegmentDuration = MP4Demuxer.readUint32(
                sidx,
                referenceIndex
            );
            referenceIndex += 4;
                ...
                if (mdhd) {
                    ...
                    if (hdlr) {
                        ...
                        if (type) {
                            ...
                            if (codecBox.length) {
                                codecBox = codecBox[0];
                                let codecType = MP4Demuxer.bin2str(
                                ...
                                logger.log(
                                    `MP4Demuxer:${type}:${codecType} found`
                                );
                            }
                            result[trackId] = {
                                timescale: timescale,
        ...
        baseTimes = [].concat.apply(
            ...
            trafs.map(function(traf) {
                return MP4Demuxer.findBox(traf, ['tfhd']).map(function(tfhd) {
                    ...
                    baseTime = MP4Demuxer.findBox(traf, ['tfdt']).map(function(
                        tfdt
                    ) {
                        var version, result;
