...
class TransportParseStream extends Stream {
    ...
    parsePmt(payload, pmt) {
        ...
        while (offset < tableEnd) {
            ...
            offset +=
                (((payload[offset + 3] & 0x0f) << 8) | payload[offset + 4]) + 5;
        }
    ...
    push(data) {
        ...
        if (!this.buffer) {
            ...
            swapBuffer = new Uint8Array(
                this.buffer.byteLength + data.data.byteLength
            );
...
class VideoSegmentStream extends Stream {
    ...
    push(data) {
            ...
            if (currentNal.nalUnitType === 'AUD') {
                ...
                sample = {
                    ...
                    compositionTimeOffset:
                        (currentNal.pts - currentNal.dts) * 90
                };
        ...
        if (this.track.samples.length) {
            sample.duration = this.track.samples[
                this.track.samples.length - 1
            ].duration;
        }
        this.track.samples.push(sample);
        this.nalUnitsLength = 0;
