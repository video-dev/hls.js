...
class Demuxer {
  ...
  constructor(hls, id) {
    ...
    // navigator is not always available in Web Worker
    // refer to https://developer.mozilla.org/en-US/docs/Web/API/WorkerGlobalScope/navigator
    // so we pass Env through postMessage
    if (config.enableWorker && (typeof (Worker) !== 'undefined')) {
      ...
      try {
        ...
        w.postMessage({ cmd: 'init', typeSupported: typeSupported, env: Env, id: id, config: JSON.stringify(config) });
      } catch (err) {
        ...
        this.demuxer = new DemuxerInline(observer, typeSupported, config, Env);
        this.w = undefined;
      }
    } else {
      this.demuxer = new DemuxerInline(observer, typeSupported, config, Env);
